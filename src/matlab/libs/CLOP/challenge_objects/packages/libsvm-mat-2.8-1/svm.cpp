...
private:
    ...
    double kernel_poly_rbf(int i, int j) const
    {
        return pow(dot(x[i],x[j])+coef0,degree)*exp(-gamma*(x_square[i]+x_square[j]-2*dot(x[i],x[j])));
    }
};
 ...
 gamma(param.gamma), coef0(param.coef0)
{
    switch(kernel_type)
    {
        ...
        case SIGMOID:
            ...
            break;
        case POLYRBF:
            kernel_function = &Kernel::kernel_poly_rbf;
            break;
    }

    clone(x,x_,l);

    if(kernel_type == RBF || kernel_type == POLYRBF )
    {
              ...
              const svm_parameter& param)
{
    switch(param.kernel_type)
    {
        ...
        case POLYRBF:
        {
            ...
            return pow(dot(x,y)+param.coef0,param.degree)*exp(-param.gamma*sum);
        }
        default:
...
{
    "linear","polynomial","rbf","sigmoid","poly_rbf",NULL
};
...
int svm_save_model(const char *model_file_name, const svm_model *model)
{
    ...
    if(param.kernel_type == POLY || param.kernel_type == POLYRBF)
        fprintf(fp,"degree %g\n", param.degree);

    if(param.kernel_type == POLY || param.kernel_type == RBF || param.kernel_type == SIGMOID || param.kernel_type == POLYRBF)
        fprintf(fp,"gamma %g\n", param.gamma);

    if(param.kernel_type == POLY || param.kernel_type == SIGMOID || param.kernel_type == POLYRBF)
        fprintf(fp,"coef0 %g\n", param.coef0);
