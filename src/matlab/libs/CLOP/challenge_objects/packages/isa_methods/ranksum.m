...
wmean = ns*(nx + ny + 1)/2;
if ~isequal(method,'approximate') & any(smsample) &  any(lgsample) % use the sampling distribution of W
    smsample=full(smsample);
    lgsample=full(lgsample);
   ...
   wc = w - wmean;
   numerator=(wc - 0.5 * sign(wc)); %IG avoid divide by zero
   denominator=sqrt(wvar);
