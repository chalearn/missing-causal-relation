function [param, idx_out]=local_KNN_train(X_train, Y_train, idx_in, flog)
%[param, idx_out]=local_KNN_train(X_train, Y_train, idx_in, flog)
...
[Y_score,K]=local_knnclass(X_train(:,idx_in), Y_train);
% Optimize the bias:
b=zeros(1,length(K));
for i=1:length(K)
