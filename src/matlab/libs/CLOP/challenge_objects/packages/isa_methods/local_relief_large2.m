function [Idxs, WS, K, D2] = local_relief_large2(X, Y, feat_num, K, group_feat, nosort, D2)
% [Idxs, WS, K, D2] = local_relief_large2(X, Y, feat_num, K, group_feat, nosort, D)
% Like Relief but uses the K nearest hits, and new normalization.
% Inputs:
...
max_num=min(min(pos_num,neg_num),16);
%if nargin<4 | isempty(K), K=2.^[0:floor(log2(max_num))]'; end
if nargin<4 | isempty(K), K=4; end % For now, we can try only one value of K at a time
...
max_val=50000; % to prevent out of memory problems 500000
dim=max(size(X,1));
...
nm=0;
K_num=length(K);
M=zeros([size(X) K_num]); % Average distances to nearest misses
H=zeros([size(X) K_num]); % Average distances to nearest hits
rnm=zeros(p,n);
rnh=zeros(p,n);
nm=zeros(p,n);
nh=zeros(p,n);
for j=1:K_num % Loop over values of K (number of neighbors)
    if j==1, mini=1; else mini=K(j-1)+1; end
	for i=mini:K(j) % Loop over the nearest hits/misses
       for k=1:length(rng)
           fprintf('nm nh j=%d, i=%d: Chunk %d\n', j, i, k);
           rnm(rng{k},:)=abs(X(rng{k},:)-X(nearest_miss(rng{k},i),:));
           rnh(rng{k},:)=abs(X(rng{k},:)-X(nearest_hit(rng{k},i),:));
       end
       nm=nm+rnm;
       nh=nh+rnh;
	end
