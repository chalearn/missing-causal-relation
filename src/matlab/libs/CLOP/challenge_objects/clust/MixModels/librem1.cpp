...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	if (arg.getmatrix(nrhs,prhs,"X",&X))
	{
		...
		if (mxIsSparse(X))
		{
			...
			int *xIndices = mxGetIr(X);
			int *jcValues = mxGetJc(X);
			double *prValues = mxGetPr(X);
	...
	//lets do the em part
	//-------------------------------------------------------
	REM em((int)mixtures,POSITIVE,BERNOULLI,gamma);

	if (!bSparse)
		em.InitTrain(Features,row,col,(int)mixtures,seedVal);
	else
		em.InitTrain(Features,row,col,(int)mixtures,seedVal,FeatureMatrix);
