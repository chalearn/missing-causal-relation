			  ...
			  const svm_parameter& param)
{
	switch(param.kernel_type)
	{
		...
		case ADPROP:
		{
			...
			double denom = num + bplusc;
			if (param.degree==0)
				return (-param.gamma*num/denom);
			else
				return exp(-param.gamma*(num/denom));
...
{
	"linear","polynomial","rbf","sigmoid","adprop",NULL
};
...
int svm_save_model(const char *model_file_name, const svm_model *model)
{
	...
	if(param.kernel_type == POLY||param.kernel_type==ADPROP)
		fprintf(fp,"degree %g\n", param.degree);

	if(param.kernel_type == POLY || param.kernel_type == RBF || param.kernel_type == SIGMOID
		||param.kernel_type == ADPROP)
		fprintf(fp,"gamma %g\n", param.gamma);
