...
//		mexPrintf("%d %d %f\n",prob.l,elements,nu);
		...
		if (mxIsSparse(x))
		{
			//can't do it
			exit(1);
			memset(x_space,0,sizeof(svm_node)*prob.l*(elements+1));
	
			int *xIndices = mxGetIr(x);
			int *jcValues = mxGetJc(x);
			double *xValues = mxGetPr(x);				
			int TotalNonZero = mxGetNzmax(x);
			int i=0,ColumnNumber=0,row=0,col=0,xspaceIndex=0;

			while (i<TotalNonZero)
			{
			}
			//now we set the flag that we have allocated the x-array
			x_allocated = true;
		}
...
//				x_space[i*(elements+1)+j].value=xdata[j*prob.l +i];
				if (xdata[j*prob.l+i]!=0)
				{	x_space[xindex].index=j+1;	
					x_space[xindex].value=xdata[j*prob.l +i];
					xindex++;
				}
			}	
...
//			x_space[i*(elements+1)+elements].value=0;
			x_space[xindex].index=-1;
			x_space[xindex].value=0;
			xindex++;
		...
		if(nlhs>1)	
		{
			...
			for(i=0;i<model->l;i++)
			{
				...
				svIndex=0;
//changed by Mehreen to be able to pass sparse data in the matrix
//				for(j=0;j<elements;j++)
//					xsv[j*model->l+i] = model->SV[i][j].value;
				for(j=0;j<elements;j++)
				{
					if (j==model->SV[i][svIndex].index-1)
					{	xsv[j*model->l+i] = model->SV[i][svIndex].value;
						svIndex++;
					}
					else
						xsv[j*model->l+i] = 0;
