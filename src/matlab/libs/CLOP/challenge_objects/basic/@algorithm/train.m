...
if isa(dat,'group') & strcmp(e.group,'separate')  
  ...
  if ~(algo.trained==1&algo.do_not_retrain==1),
	if algo.verbosity==0
        ...
        %IG: in case of absence of the training method. Nov 2, 05
        if ~ismethod(algo, 'training')
            algor=algo;
            warning('The training is not defined for this object.');
        else
...
%%% IG: value of Y used to break ties
if ~isa(dat, 'group') & isa(dat, 'data')
    pnum=length(get_y(dat)>0);
    nnum=length(get_y(dat)<0);
    if pnum>nnum
        algor.default_output=1;
    else
        algor.default_output=-1;
    end
end