...
%  Usage: svc_plot(X,Y,ker,q,gamma,alpha,bias,aspect,mag,xaxis,yaxis,input,Xtest,Ytest)
%
...
%  Modified: Isabelle Guyon - August/September 1999 - 2008 - isabelle@clopinet.com -
    ...
    sv_X = X(sv,:); sv_Y = Y(sv);
    % All dot products:
    %DP = svc_dp(ker,input,sv_X,q,gamma);
    ...
    z = reshape(Z,size(x,1),size(x,2)); %don't add bias??
    ...
    for i=1:size(alpha)
        plot(a.Xsv.X(i,xaxis),a.Xsv.X(i,yaxis),'wo') % Support Vector
    end
    for i = 1:size(Ytest)
      if (Ytest(i) == 1)
