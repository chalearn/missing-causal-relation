...
if length(hyper)==1 && (isa(hyper,'kernel') || isa(hyper,'distance') || isa(hyper,'algorithm') || isa(hyper,'data'))
      a.child=hyper;
else
% ------------ evaluate input --------------

 for i=1:size(hyper,2)
      if iscell(hyper{i}) || isa(hyper{i},'kernel') || isa(hyper{i},'distance') || isa(hyper{i},'algorithm') || isa(hyper,'data') || (isnumeric(hyper{i}) & length(hyper{i})>1)
           a.child=hyper{i};
      else
           value=hyper{i};
           if ischar(value)
