...
if isempty(Y)    dtmp = {};    for i = 1:inplen        dtmp{i} = partition(inp{i},sel);    end    d1 = joint_data(dtmp);else    dtmp = {};    for i = 1:inplen        dtmp{i} = partition(inp{i},sel);    end    d1 = joint_data(dtmp,Y(sel,:));end
if nargout < 2, return, endif isempty(Y)    dtmp = {};    for i = 1:inplen        dtmp{i} = partition(inp{i},~sel);    end    d2 = joint_data(dtmp);else    dtmp = {};    for i = 1:inplen        dtmp{i} = partition(inp{i},~sel);    end    d2 = joint_data(dtmp,Y(~sel,:));end
