function data_square_plot(a,d2,granul)
%
...
if 1
for i=sv'
 col='rs'; if a.alpha(i)<0 col='bs'; end;
 %if (r.X(i,:)*r.Y(i,:)<0.9) col='cs';   end;  %% margin error 
 h=plot(x(i,1),x(i,2),col);
 alpha=2; %ceil((abs(a.alpha(i))/amax)*4); 
 set(h,'LineWidth',alpha,'MarkerSize',t+8+alpha);
end
end

 h=plot(d2.X(d2.Y==1,1),d2.X(d2.Y==1,2),'rx');
 set(h,'LineWidth',2,'MarkerSize',t+7);
 h=plot(d2.X(d2.Y==-1,1),d2.X(d2.Y==-1,2),'bo');
 set(h,'LineWidth',2,'MarkerSize',t+7);
